 ...
 * NuProcess, and as a result you should never perform a blocking or
 * time-consuming operation on the callback thread. Doing so will block or
 * severely hinder the processing of other NuProcess instances. Any operation
 * that requires more than single-digit millisecond processing should be handed
 * off to another thread so that the processing thread can return to immediately
 * service other NuProcess instances.
 * 
...
public interface NuProcessHandler
{
    ...
    * method. This is an opportunity to store away the {@code NuProcess}
    ...
    * 
    * @param nuProcess The {@link NuProcess} that is starting. Note that the
    ...
    *        {@link #onStart(NuProcess)} instead.
    */
    ...
    * interaction within other callbacks in this interface.
    * <p>
    * Note that this method is called <em>at some point after</em> the process is
    * spawned. It is possible for other methods (even {@link #onExit(int)}) to
    * be called first. If you need a guarantee that no other methods will be
    * called first, use {@link #onPreStart(NuProcess)} instead.
    *
    ...
    * {@link #onStdout(ByteBuffer, boolean)} method will be called twice for an
    ...
    * {@link #onStdout(ByteBuffer, boolean)} method would only be called once
